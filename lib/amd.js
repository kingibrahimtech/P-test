...
async function syncdb() {
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec = "\nBLACK PANTHER MD ONLINE\n\n  Prefix  : [ " + (prefix ? prefix : "null") + " ]\n  Plugins : " + events.commands.length + "\n  Mode    : " + Config.WORKTYPE + "\n  Database: " + (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "IBRAHIM-SDV") + "\n";
      try {
...
<div class="bounce-text">IBRAHIM ADAMS</div>
...
app.get("/Panther", (_0x2e8c1a, _0x30322c) => _0x30322c.type("html").send(html));
...
app.listen(quickport, () => console.log("Panther On  http://localhost:" + quickport + "/  "));
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^PANTHER_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/PANTHER;;;/gi, "").replace(/Panther;;;/gi, "").replace(/kingibrahimtech;;;/gi, "").trim();
  ...
  const _0x406ff0 = "/kingibrahimtech/";
